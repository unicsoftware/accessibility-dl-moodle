...
# Arquivo: src/training/train_gradient_boosting.py
# Descrição: Treinamento do modelo Gradient Boosting.
# =====================================================================
"""
Script de treinamento do Gradient Boosting.

Uso:
    python src/training/train_gradient_boosting.py [--seed 42]
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Treina o modelo Gradient Boosting.")
...
def main() -> None:
    ...
    # Treinamento
    model = GradientBoostingAccessibilityModel(random_state=args.seed)
    model.fit(X_train, y_train)
    print("[INFO] Modelo Gradient Boosting treinado.")
    ...
    # Persistência
    model.save(GB_MODEL_FILE)
    print(f"[INFO] Modelo salvo em {get_rel_path(GB_MODEL_FILE)}")
    ...
    export_dataframe(predictions_df, ROOT / "results" / "predictions_gb.csv")

    # Salva métricas e metadados
    export_metrics({"gradient_boosting": metrics["test"]}, METRICS_FILE)
    export_metadata(
        {
